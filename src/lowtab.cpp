...
// 低 t 段逐点重算：一次 LP 排序（按 T2 视野），随后对 T1..T2 每个时刻做 LP 值束搜索。
int main(int argc,char**argv){
    if(argc<7){printf("用法: lowtab <beam.bin> <T1> <T2> <K> <BEAMB> <out.txt> [DELW]\n");return 1;}
    const char*fn=argv[1]; int T1=atoi(argv[2]), T2=atoi(argv[3]);
    int K=atoi(argv[4]); int BEAMB=atoi(argv[5]); const char*outp=argv[6];
    int DELW=argc>7?atoi(argv[7]):1;
    // 断点续算：读已有输出
    set<int> done;
    { FILE*fi=fopen(outp,"r"); if(fi){ char ln[256];
        while(fgets(ln,sizeof(ln),fi)){ int tt; unsigned long long vv;
            if(ln[0]=='#') continue;
            if(sscanf(ln,"%d %llu",&tt,&vv)==2) done.insert(tt); }
        fclose(fi);} }
    ...
    fclose(f);
    if(T2<=t0){printf("T2<=t0\n");return 1;}
    fprintf(stderr,"%s: %d 状态 t0=%d, 目标 %d..%d\n",fn,cnt,t0,T1,T2);
    // 排序视野取中点，兼顾整个区间
    int HzR=(T1+T2)/2-t0;
    vector<pair<D,int> > rk(cnt);
    #pragma omp parallel for schedule(dynamic,4)
    for(int i=0;i<cnt;i++){ D v=lpBound(all[i],HzR,0); rk[i]=make_pair(v,i); }
    ...
    fprintf(stderr,"LP 排序完毕, 取前 %d 个\n",lim);
    { FILE*fo=fopen(outp,"a"); fprintf(fo,"# %s t0=%d 束宽=%d DELW=%d 种子=%d\n",fn,t0,BEAMB,DELW,lim); fclose(fo); }
    for(int T=T1;T<=T2;T++){
        if(done.count(T)) continue;
        vector<array<u64,3> > A; int bi2=-1;
        u64 v=lpBeam(init,T-t0,BEAMB,DELW,&A,&bi2,0);
        int bs; bool okv=(bi2>=0)&&verify(all[rk[bi2].second],A,v,bs);
        FILE*fo=fopen(outp,"a"); fprintf(fo,"%d %llu %s\n",T,2*v,okv?"ok":"BAD"); fclose(fo);
