...
int main(int argc,char**argv){
    if(argc<7){printf("用法: improve <in.bin> <TG> <T> <B> <NOUT> <out.bin> [DP DK DQ]\n");return 1;}
    const char*fn=argv[1]; int TG=atoi(argv[2]); int T=atoi(argv[3]);
    int B=atoi(argv[4]); int NOUT=atoi(argv[5]); const char*of=argv[6];
    ...
    printf("%s: %d 状态 tb=%d -> TG=%d  打分时域 T=%d  束宽=%d  候选(%d,%d,%d)\n",fn,cnt,tb,TG,T,B,DP,DK,DQ);
    printf("基线(纯成长) x(%d)*2 = %llu\n",T,2*base);
    fflush(stdout);
    // 先把初始集裁到束宽
    ...
    for(int t=tb;t<TG;t++){
        ...
        vector<St> ns(kk); vector<vector<array<u64,3> > > nh(kk);
        for(int i=0;i<kk;i++){ int id=sc[i].second; ns[i]=nx[id];
            nh[i]=hist[src[id].first]; nh[i].push_back(src[id].second); }
        cur.swap(ns); hist.swap(nh);
        if((t-tb)%20==0||t==TG-1){ printf("  t=%d 候选=%d 束=%d 最好成长值*2=%llu  (%.0fs)\n",
            t+1,n,(int)cur.size(),2*sc[0].first,omp_get_wtime()-w0); fflush(stdout); }
    }
    u64 bestv=growTo(cur[0],TG,T);
    printf("改进后 x(%d)*2 = %llu   相对基线 %+.4Le\n",T,2*bestv,(double)((long double)bestv-(long double)base)/(long double)base);
    int kk=min(NOUT,(int)cur.size());
    FILE*fo=fopen(of,"wb"); fwrite(&kk,4,1,fo); fwrite(&TG,4,1,fo);
    for(int i=0;i<kk;i++) fwrite(&cur[i],sizeof(St),1,fo); fclose(fo);
    printf("写出 %d 个 t=%d 状态 -> %s\n",kk,TG,of);
    if(getenv("ACTOUT")){ FILE*fa=fopen(getenv("ACTOUT"),"w");
        fprintf(fa,"%d %d %d\n",tb,kk,(int)hist[0].size());
        for(int z=0;z<kk;z++){ for(size_t i=0;i<hist[z].size();i++)
            fprintf(fa,"%llu %llu %llu\n",hist[z][i][0],hist[z][i][1],hist[z][i][2]); }
        fclose(fa); printf("动作序列 -> %s (%d 个状态 x %d 步, 起点 t=%d)\n",getenv("ACTOUT"),kk,(int)hist[0].size(),tb); }
    return 0;
}
